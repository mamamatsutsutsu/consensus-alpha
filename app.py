...
# =========================
# 1. Phantom UI Configuration (Font Updated)
# =========================
...
html, body, .stApp { background-color: var(--bg-app) !important; color: var(--text-main) !important; font-family: 'Inter', sans-serif !important; }

/* ORBITRON FONT OVERRIDES (Making it cool) */
h1, h2, h3, .brand-title, .kpi-val, .big-num, .stMetricValue, div[data-testid="stMetricValue"], .badge {
...
.brand-sub { font-family: 'Orbitron', sans-serif; font-size: 12px; color: var(--accent); letter-spacing: 3px; opacity: 0.8; margin-top: 5px; }
...
.badge { display:inline-block; padding:3px 10px; border-radius:4px; font-size:10px; font-weight:700; border:1px solid; margin-right:5px; }
.b-strong { border-color:#1f6feb; color:#58a6ff; background:rgba(31,111,235,0.15); }
.b-watch { border-color:#d29922; color:#f0b429; background:rgba(210,153,34,0.15); }
.b-avoid { border-color:#da3633; color:#f85149; background:rgba(218,54,51,0.15); }
...
def check_auth():
    ...
    with col2:
        ...
        with st.form("login"):
            st.markdown("<h3 style='text-align:center; font-family:Orbitron;'>🔒 SYSTEM LOCKED</h3>", unsafe_allow_html=True)
...
# STOCK LISTS (Partial for brevity, assumes logic expands this or user provides more)
# ... (Previous Full Lists are maintained in logic, abbreviated here for code block limit but fully loaded in NAME_DB)
US_STOCKS = {
    ...
    "Healthcare": ["LLY","UNH","JNJ","ABBV","MRK","TMO","ABT","AMGN","PFE","ISRG","DHR","VRTX","GILD","REGN","BMY","CVS","CI","SYK","BSX","MDT"],
    "Financials": ["JPM","BAC","WFC","V","MA","AXP","GS","MS","BLK","C","SCHW","SPGI","PGR","CB","MMC","KKR","BX","TRV","AFL","MET","PRU","ICE"],
    "Cons. Disc": ["AMZN","TSLA","HD","MCD","NKE","SBUX","LOW","BKNG","TJX","CMG","MAR","HLT","YUM","LULU","GM","F","ROST","ORLY","AZO","DHI"],
    "Cons. Staples": ["PG","KO","PEP","COST","WMT","PM","MO","MDLZ","CL","KMB","GIS","KHC","KR","STZ","EL","TGT","DG","ADM","SYY"],
    "Industrials": ["GE","CAT","DE","HON","UNP","UPS","RTX","LMT","BA","MMM","ETN","EMR","ITW","WM","NSC","CSX","GD","NOC","TDG","PCAR","FDX"],
...
def check_gemini_health():
    # Diagnostic to display in sidebar
    if not HAS_GENAI_LIB: return "LIBRARY MISSING"
    if not API_KEY: return "KEY MISSING"
    return "ONLINE"
...
def call_gemini(ticker: str, name: str, stats: Dict) -> str:
    ...
    if HAS_GENAI_LIB and API_KEY:
        try:
            ...
            prompt = f"""
            あなたはプロの機関投資家です。以下の銘柄について「モメンタム」「リスク」「マクロ」の3名のエージェントとして議論し、結論を出してください。
            日本語で、専門的かつ断定的に。
            
            銘柄: {name} ({ticker})
            [指標] RS:{stats['RS']:.2f}%, Accel:{stats['Accel']:.2f}, DD:{stats['MaxDD']:.2f}%, Stable:{stats['Stable']}
            [騰落] 1M:{stats['1M']:.1f}%, 12M:{stats['12M']:.1f}%
    ...
    # Fallback Rule-based (Show reason)
    reason = "ライブラリ未導入" if not HAS_GENAI_LIB else "APIキー未設定"
    view = "強気" if stats['RS']>0 and stats['Accel']>0 else "中立"
    return f"※AI利用不可 ({reason})\n\n【自動判定】トレンド: {view}\nRS: {stats['RS']:.2f}% | 12M: {stats['12M']:.1f}%"
...
def main():
    # Sidebar Diagnostic
    with st.sidebar:
        st.caption(f"AI Status: {check_gemini_health()}")
        if not API_KEY: st.warning("Set GEMINI_API_KEY in secrets to enable AI.")
    ...
    if bench not in audit["list"]:
        st.error(f"Benchmark {bench} unavailable. Check connection.")
        st.stop()
    ...
    with col2: st.markdown(f"<div class='kpi-box status-green'><div class='kpi-label'>Mode Date</div><div class='kpi-val'>{str(audit['mode']).split()[0]}</div></div>", unsafe_allow_html=True)
