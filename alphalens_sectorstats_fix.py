...
# Sector Stats (safe)
try:
    if isinstance(df, pd.DataFrame) and (len(df) > 0) and ("RS" in df.columns) and ("Ret" in df.columns):
        _med_rs = float(df["RS"].median())
        _med_ret = float(df["Ret"].median())
        _spr_rs = float(df["RS"].max() - df["RS"].min())
        sector_stats_str = f"Universe:{len(stock_list)} Computable:{len(df)} MedianRS:{_med_rs:.2f} MedianRet:{_med_ret:.1f}% SpreadRS:{_spr_rs:.2f}"
    else:
        sector_stats_str = f"Universe:{len(stock_list)} Computable:{len(df) if isinstance(df, pd.DataFrame) else 0}"
except Exception:
    sector_stats_str = f"Universe:{len(stock_list)} Computable:{len(df) if isinstance(df, pd.DataFrame) else 0}"
